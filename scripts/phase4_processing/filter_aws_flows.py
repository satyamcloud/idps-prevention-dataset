...
df = pd.read_csv("data/processed/aws_flows.csv", low_memory=False)
print(f"Before filtering: {len(df)} flows")

df['flow_start'] = pd.to_datetime(df['bidirectional_first_seen_ms'], unit='ms')

start_window = pd.Timestamp("2026-09-04 17:02:31")
end_window = pd.Timestamp("2026-09-04 21:25:31")
...
filtered.to_csv("data/processed/aws_flows_filtered.csv", index=False)
print("Saved to data/processed/aws_flows_filtered.csv")
