...
def run_beacon(session_num, attack_type, beacon_count, interval, severity):
    ...
    try:
        ...
        end_ts = now_utc()
        log_attack_session(
            session_id=session_id, attack_category="botnet_beacon",
            tool="curl", command=f"{command} (x{beacon_count}, every {interval}s)",
            start_ts=start_ts, end_ts=end_ts, source_ip=SOURCE_IP, target_ip=TARGET_IP,
            cloud=CLOUD, target_port=80, intended_severity=severity, notes=outcome_note
        )
